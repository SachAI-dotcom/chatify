...
const generateRefreshTokenAndAccessToken = async(req,res)=>{
    ...
    const refreshToken = jsonwebtoken.sign({id: user._id}, process.env.JWT_SECRET, {expiresIn: '7d'});
    const accessToken = jsonwebtoken.sign({id: user._id}, process.env.JWT_SECRET, {expiresIn: '15m'});
    return new ApiResponse(200, {refreshToken, accessToken}, 'Tokens generated successfully').send(res);
}
